...
public:
    ...
    math::Vec2d projection(math::Vec3d const & p3d){
        ...
        //define R and t in matrix or vector form
        ...
        K(2, 1) = 0;
        K(2, 2) = 1.0;
        // camera coord to uniform image coord
        math::Vec3d p_i = 1.0/p_c[2] * K.mult(p_c);

        // correct the distortion
        double r2 = std::pow(p_i[0], 2) + std::pow(p_i[1], 2);
        p[0] = p_i[0] * (1 + dist_[0] * r2 + dist_[1] * std::pow(r2, 2));
        p[1] = p_i[1] * (1 + dist_[0] * r2 + dist_[1] * std::pow(r2, 2));
        return p;
