...
FundamentalMatrix fundamental_8_point (math::Matrix<double, 3, 8> const& points1
    ...
    // svd Af = 0
    math::Matrix<double, 9, 9> V;
    math::matrix_svd<double, 8, 9>(F_raw, nullptr, nullptr, &V);
    /* prove : the optimal f is the eigenvector with smallest eigen-value of F_raw.transpose() * F_raw
    ...
    */
    ...
    math::Matrix<double, 3,3> U, Sig, Vnew;
    math::matrix_svd<double, 3, 3>(F, &U, &Sig, &Vnew);
    Sig(2,2) = 0; // force the smallest singular value to be 0 ;
    F = (U.mult(Sig)).mult(Vnew.transpose());
    return F;
#if 0
